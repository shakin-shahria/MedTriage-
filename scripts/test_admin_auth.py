...
# Ensure project root is on sys.path so imports like `import main` work when
# running this script from the scripts/ directory or elsewhere.
...
def run_tests():
    ...
    try:
        main.admin_sessions(request=req, x_admin_token=None)
    ...
    try:
        out = main.admin_sessions(request=req, x_admin_token=None)
