...
from ml_triage import try_heart_attack_triage
import os
import logging
import time
...
async def lifespan(app: FastAPI):
    ...
    Preloads ML model when MEDTRIAGE_PRELOAD_ML=1 to avoid model downloads in
    request handlers.
    """
    ...
    if preload == "1":
        try:
            _ml._init()
            logger.info("ML model preloaded at startup via lifespan handler")
        except Exception:
            logger.exception("Failed to preload ML model at startup; continuing with rule-based fallback")
    yield


app.router.lifespan_context = lifespan  # set lifespan for the app
...
class HeartTriageRequest(BaseModel):
    age: int
    sex: int
    cp: int
    trestbps: float
...
class HeartTriageResponse(BaseModel):
    prediction: str
    confidence: float
...
@app.post("/triage_heart", response_model=HeartTriageResponse)
def triage_heart(req: HeartTriageRequest, request: Request = None):
    ...
    try:
        ...
        data = req.dict()
    # Try the specialized heart model
    ...
    try:
        BAND_LOW = float(os.environ.get('MEDTRIAGE_HEART_BAND_LOW', '0.10'))
        BAND_MED = float(os.environ.get('MEDTRIAGE_HEART_BAND_MED', '0.40'))
        ABSTAIN_LOW = float(os.environ.get('MEDTRIAGE_HEART_ABSTAIN_LOW', '0.45'))
        ABSTAIN_HIGH = float(os.environ.get('MEDTRIAGE_HEART_ABSTAIN_HIGH', '0.55'))
    except Exception:
        BAND_LOW, BAND_MED, ABSTAIN_LOW, ABSTAIN_HIGH = 0.10, 0.40, 0.45, 0.55

    # keep copy of model outputs in case we fallback, so we can preserve model
    # confidence and matches for transparency
    ...
    if failed or pred is None:
        ...
        # Model produced a result: conf is expected to be calibrated probability
        # If probability lies within the abstain window, treat as inconclusive
        if conf is None:
            fallback = True
    ...
    try:
        if fallback:
            ...
            # map pred into standardized labels
            if isinstance(pred, str) and 'heart' in pred.lower():
                db_risk = 'high'
            elif isinstance(pred, str) and 'moder' in pred.lower():
                db_risk = 'medium'
            else:
                db_risk = 'low'
    except Exception:
    ...
    try:
        ...
        logger.exception("Failed to record heart triage session")

    important = []
    try:
        # Convert matches to list[str]
        important = [str(m) for m in matches] if matches else []
    except Exception:
        important = []

    return HeartTriageResponse(prediction=pred, confidence=float(conf), important_features=important, suggestion=(suggestion or None))
...
@app.get("/admin/users")
def admin_users(x_admin_token: Optional[str] = Header(None), request: Request = None):
    """Return all users. Guarded by admin authentication."""
    # Reuse the same authorization logic as admin_sessions
    authorised = False
    expected_token = os.environ.get("MEDTRIAGE_ADMIN_TOKEN")
