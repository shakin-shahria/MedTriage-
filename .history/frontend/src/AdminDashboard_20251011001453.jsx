...
export default function AdminDashboard(){
  ...
  async function fetchSessions(){
    ...
    try{
      ...
      setAnalytics({ totalUsers: analytics.totalUsers || 0, totalSessions, highRisk: highs, avgConfidence: avgConf ? Number(avgConf.toFixed(2)) : 0 })
              ...
              {sessions && sessions.length>0 ? sessions.map(s=> (
                <div key={s.session_id} className="p-3 border rounded flex justify-between items-start">
                  <div>
                    ...
                    <div className="font-medium text-gray-900">{s.user_email || s.user || '—'}</div>
