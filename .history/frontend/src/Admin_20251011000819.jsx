...
export default function Admin() {
  ...
  useEffect(() => {
    try {
      ...
      if (t) {
        ...
        // If a regular JWT is present and the user is an admin, use it as a fallback
        setIsLoggedIn(true)
        setShowToken(false)
      }
    } catch (e) {
  ...
  async function fetchSessions(){
    ...
    try{
      ...
      // prefer persisted admin token; if not present, allow regular Bearer JWT from 'token'
      let tokenToUse = (typeof window !== 'undefined' && localStorage.getItem(STORAGE_KEY)) || adminToken
      if (!tokenToUse) {
        try {
          const bearer = localStorage.getItem('token')
          if (bearer) tokenToUse = `Bearer ${bearer}`
        } catch (e) {
          // ignore
        }
      }
      ...
      if(!res.ok){
        if(res.status === 401) throw new Error('Unauthorized: please login as admin or provide a valid token')
        if(res.status === 403) throw new Error('Forbidden: your account does not have admin privileges')
