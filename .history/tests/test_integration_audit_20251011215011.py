...
def test_audit_called_with_fallback_when_model_missing():
    ...
    # Patch main to enable DB and return a fake DB session generator
    def fake_get_db():
        try:
            yield object()
        finally:
    ...
    with patch.object(main, 'crud', dummy), patch.object(main, 'get_db', fake_get_db), patch.object(main, 'DB_ENABLED', True):
